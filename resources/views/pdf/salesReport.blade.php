...
<body>
    ...
    @if($devoluciones->count()>0)
        <div style="background-color:#FEDEE5;width:100%;color:#863040;">Existen <strong>{{ $devoluciones->count() }} devoluciones</strong> realizadas el día de hoy!</div>
    @endif
        ...
        <tbody class="text-center" style="">
              ...
              @foreach ($reportData as $invoice)
                ...
                <tr style="border-color:black;background-color:lightgreen">
                    ...
                    <td class="d-none d-md-table-cell" style="font-size:14px;">@if($invoice->client)
					...
					<td class="text-left" style="font-size:14px;">{{ number_format($invoice->descuento,2,',','.') }}%</td>
                    <td class="text-left" style="font-size:14px;">Bs. {{ number_format($invoice->total,2,',','.') }}</td>
                    ...
                    @if($invoice->pay->divisa>0)
                        ...
                        <span style="color:red"><strong> Divisa:</strong> $. {{ $invoice->pay->divisa }}</span><br>
                            
                    @endif
                    @if($invoice->pay->zelle>0)
                     
                                <span style="color:darkred;"><strong>Zelle:</strong> $. {{ $invoice->pay->zelle }} Ref: {{ $invoice->pay->refZelle }}<br>
                     
                    @endif
                    @if($invoice->pay->Bolivares>0)
                     
                                <span style="color:darkmagenta;"><strong>Bs Efec:</strong>  Bs. {{ $invoice->pay->Bolivares }}</span><br>
                    ...
                    @if($invoice->pay->Credito>0)
                                ...
                                <span style="color:darkgreen;"><strong>Credito:</strong> Bs. {{ $invoice->pay->Credito }} Ref: {{ $invoice->pay->refCredito }}</span><br>
                     
                    @endif
                    @if($invoice->pay->Transferencia>0)
                        
                        <span style="color:darkblue"><strong>Transf:</strong> Bs. {{ $invoice->pay->Transferencia }} Ref: {{ $invoice->pay->refTransferencia }}</span><br>
   ...
   <htmlpagefooter  name="page-footer" >
        <div id="footer" style="width: 100%;text-align:center;">
            <span style="font-size:14px;">Facturation System C.A.</a> Copyright ©2020 Todos los derechos reservados</span><br>
