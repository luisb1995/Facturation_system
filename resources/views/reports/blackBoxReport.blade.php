...
<body>
    ...
    <header style="width: 100%; margin-top: 0px; margin-bottom: 10px;height:250px;">
        <div style="float:left;">
            <img src="{{ asset('img/logo50.png') }}" alt=""class="img" height="50">
        </div>
        <div style="float:left;margin-left:65px;">
            <h2 ><strong>Reporte de auditoría</strong></h2>
        </div>
        <div style="float:left;margin-left:65px;">
   ...
   <div>
    <table class="table table-striped">
        ...
        <tbody class="text-center">
              ...
              @foreach ($reportData as $register)
                <tr style="border-color:black;">
                    <td>{{ $register->type }}</td>
                    <td>{{ $register->action }}</td>
                    <td style="text-align:left;">{!! $register->details !!}</td>
                    <td>{{ $register->user }}</td>
                    <td class="d-none d-md-table-cell">{{ $register->created_at }}</td>
                </tr>
